...
parser.add_argument("path", metavar="PATH", type=str,
                    help="relative album path or substring")
parser.add_argument('-a', '--all', action='store_true', help="merge tags and rating between all items in group")
parser.add_argument('-t', '--tags', action='store_true', help="clone tags from parent to children")
...
"""
sqlRating = """
SELECT MAX(rating) FROM ImageInformation WHERE imageid IN %(ids)s;
"""
sqlRatingsUpdate = """
...
groups = []
for row in cur:
    groups.append({"prefix":row[0], "album":row[1], "path":row[2]})
...
for group in groups:
    ...
    if (len(imgs) > 1):
        sql = sqlGroupIns
        subs = imgs.copy();
        obj=subs.pop(0); # shift first element
        names = list(str(i["name"]) for i in subs)
        print("\t{0} ({1})".format(obj["name"],", ".join(names)))
        ins = []
        for img in subs:
            ins.append("({obj}, {sub}, {type})".format(obj=obj["id"],sub=img["id"],type=groupType))
        ...
        if (args.tags):
            ...
            for img in subs:
                ...
                cur = db.execute(sqlTagsAdditional, {'obj': obj["id"], 'sub': img["id"]})
                extra = False
                # report on additional tags
                for row in cur:
                    if not extra:
                        print("\t    Child has additional tags, not cloning")
                    print("\t    {0} - {1}".format(img["name"], row[1]))
                    extra = True
                # copy tags
                if extra == False:
